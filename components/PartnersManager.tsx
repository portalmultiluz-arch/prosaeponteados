...
interface PartnersManagerProps {
  onBack: () => void;
  items: PartnerItem[];
  onItemsChange: (newItems: PartnerItem[]) => void;
}

const PartnersManager: React.FC<PartnersManagerProps> = ({ onBack, items, onItemsChange }) => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [currentItem, setCurrentItem] = useState<PartnerItem | null>(null);
  const [formState, setFormState] = useState<Omit<PartnerItem, 'id'>>({
    title: '', description: '', imageUrl: '', videoUrl: '', textUrl: ''
  });

  const openModal = (item: PartnerItem | null = null) => {
    setCurrentItem(item);
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-bold text-gray-800">Gerenciar Parceiros e Anunciantes</h2>
        <button
          onClick={() => openModal()}
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full bg-white border">
          ...
          <tbody className="text-gray-700">
            ...
            {items.length === 0 && (
              <tr><td colSpan={2} className="text-center py-4 text-gray-500">Nenhum parceiro cadastrado.</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl p-6 relative animate-fade-in-scale max-h-[90vh] overflow-y-auto">
            <h3 className="text-lg font-semibold mb-4">{currentItem ? 'Editar' : 'Adicionar'} Parceiro</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <input type="url" name="textUrl" value={formState.textUrl || ''} onChange={handleFormChange} placeholder="URL do Site/Artigo (Opcional)" className="w-full rounded-md border-gray-300" />
