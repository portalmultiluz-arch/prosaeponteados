...
import type { DemystifyAIItem } from '../types';
...
const ContentCard: React.FC<{ item: DemystifyAIItem, onOpenSubscriptionModal: () => void }> = ({ item, onOpenSubscriptionModal }) => (
    <div className="bg-white rounded-lg shadow-lg overflow-hidden">
        ...
        <div className="p-6">
            ...
            <div className="flex flex-wrap gap-4 border-b pb-6 mb-6">
                ...
                {item.textUrl && <a href={item.textUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 bg-blue-100 text-blue-800 font-bold py-2 px-4 rounded-lg hover:bg-blue-200 transition-colors"><BookOpenIcon className="h-5 w-5"/> Ler Artigo</a>}
            </div>
            <div className="text-center bg-slate-50 p-4 rounded-lg">
                <h4 className="font-semibold text-lg text-brand-primary">Gostou do conteúdo? Saiba mais!</h4>
                <p className="text-gray-600 my-2 text-sm">Nossos assinantes têm acesso a conteúdos exclusivos, aulas ao vivo e muito mais. Apoie a cultura brasileira e aprofunde seus conhecimentos.</p>
                <button 
...
const DemystifyAIPage: React.FC<PageProps> = (props) => {
            ...
            <main className="flex-grow py-16 md:py-20">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="text-center mb-8">
                        <h1 className="text-4xl font-serif text-brand-primary">Desmistificação da IA</h1>
                        <p className="mt-4 text-lg text-gray-600 max-w-3xl mx-auto">Entenda como a Inteligência Artificial pode ser uma aliada na preservação e difusão da cultura.</p>
                    </div>
                    ...
                    <div className="space-y-12">
                        {items.length > 0 ? items.map(item => <ContentCard key={item.id} item={item} onOpenSubscriptionModal={onOpenSubscriptionModal} />) : (
                            <div className="text-center bg-white p-10 rounded-lg shadow-md"><h3 className="text-xl font-semibold text-gray-700">Nenhum conteúdo disponível.</h3></div>
                        )}
                    </div>
                </div>
            </main>
...
export default DemystifyAIPage;