...
import { MailIcon, SubscriptionIcon, XIcon, VideoCameraIcon } from './icons';

interface CulturalKnowledgePageProps {
...
const ContentCard: React.FC<{ item: CulturalKnowledgeItem, onSelect: () => void }> = ({ item, onSelect }) => (
    <div 
    ...
    >
        ...
        <div className="p-5">
            <span className="inline-block bg-brand-accent/20 text-brand-primary text-xs font-semibold px-2 py-1 rounded-full mb-2">{item.type}</span>
            <h3 className="text-xl font-bold font-serif text-brand-primary">{item.title}</h3>
            <p className="text-gray-600 mt-1 text-sm">{item.tagline}</p>
        </div>
    </div>
);

const ContentModal: React.FC<{ item: CulturalKnowledgeItem, onClose: () => void, onOpenSubscriptionModal: () => void }> = ({ item, onClose, onOpenSubscriptionModal }) => (
    <div 
    ...
    >
        ...
        >
            ...
            <div className="flex-grow overflow-y-auto p-8">
                <span className="inline-block bg-brand-accent/20 text-brand-primary text-sm font-semibold px-3 py-1 rounded-full mb-3">{item.type}</span>
                <h2 className="text-3xl font-serif text-brand-primary mb-4">{item.title}</h2>
            ...
            <div className="flex-shrink-0 mt-auto p-6 bg-white border-t flex flex-wrap gap-4 justify-end items-center">
                <a 
                    href="mailto:prosaeponteado@gmail.com?subject=Interesse no conteúdo"
...
const CulturalKnowledgePage: React.FC<CulturalKnowledgePageProps> = ({ 
    onNavigateToMain, onNavigateToAdmin, isLoggedIn, onLogout, onOpenSubscriptionModal, culturalKnowledge,
    theme, onThemeChange, fontSize, onFontSizeChange
}) => {
    const [selectedItem, setSelectedItem] = useState<CulturalKnowledgeItem | null>(null);
            ...
            <main className="flex-grow py-16 md:py-20">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="text-center mb-12">
                        <h1 className="text-4xl font-serif text-brand-primary">O Saber Cultural</h1>
                        <p className="mt-4 text-lg text-gray-600 max-w-3xl mx-auto">
                           Uma galeria de artigos, vídeos e materiais para aprofundar sua conexão com as diversas facetas da cultura brasileira.
                        </p>
                         <button 
                     ...
                     {culturalKnowledge.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            {culturalKnowledge.map(item => (
                                <ContentCard key={item.id} item={item} onSelect={() => setSelectedItem(item)} />
                            ))}
                        </div>
                    ) : (
                         <div className="text-center bg-white p-10 rounded-lg shadow-md">
                            <h3 className="text-xl font-semibold text-gray-700">Nenhum conteúdo disponível.</h3>
                            <p className="text-gray-500 mt-2">Nossa galeria cultural está sendo preparada. Volte em breve para conferir as novidades!</p>
                        </div>
                    )}
                </div>
            </main>
            <Footer onNavigateToAdmin={onNavigateToAdmin} />
            {selectedItem && <ContentModal item={selectedItem} onClose={() => setSelectedItem(null)} onOpenSubscriptionModal={onOpenSubscriptionModal} />}
        </div>
    );
};
