...
interface DemystifyAIManagerProps {
  onBack: () => void;
  items: DemystifyAIItem[];
  onItemsChange: (newItems: DemystifyAIItem[]) => void;
}

const DemystifyAIManager: React.FC<DemystifyAIManagerProps> = ({ onBack, items, onItemsChange }) => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [currentItem, setCurrentItem] = useState<DemystifyAIItem | null>(null);
  const [formState, setFormState] = useState<Omit<DemystifyAIItem, 'id'>>({
    title: '', description: '', imageUrl: '', videoUrl: '', textUrl: ''
  });

  const openModal = (item: DemystifyAIItem | null = null) => {
    setCurrentItem(item);
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-bold text-gray-800">Gerenciar "Desmistificação da IA"</h2>
        <button
          onClick={() => openModal()}
...
export default DemystifyAIManager;